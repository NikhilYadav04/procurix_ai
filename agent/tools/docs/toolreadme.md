...
The RFP tool uses PDFKit to generate professional, styled PDF documents with a premium amber theme. The design includes a cover page, executive summary, project details, requirements sections, and contact information cards—all with consistent typography and visual hierarchy.

The implementation is in `agent/tools/rfptool.tsx` and generates 5-page documents that work reliably in both local development and serverless deployment environments (like Vercel). No browser dependencies are required.

If you wish to modify the visual style, update the color palette and layout code in the `generateRfpPdf` function.
# Tools
...
The `create_rfp` tool generates a professional RFP PDF from structured input and saves it to the `agent/tools/rfps` folder (no external upload or DB persistence).

Use it like other tools in `langchain` — with a `zod` schema for validation. See `agent/tools/rfptool.tsx` for the implementation.
